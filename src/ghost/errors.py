...
class UnparseableCodeError(GhostError):
    """Generated test code is not valid Python syntax."""

    def __init__(self, source_path: Path | str, error: str) -> None:
        self.source_path = source_path
        self.error = error
        super().__init__(f"LLM generated invalid Python syntax for {source_path}: {error}")


class HandwrittenTestOverwriteError(GhostError):
    """Refusing to overwrite an existing test that was not generated by Ghost."""

    def __init__(self, test_file: Path | str) -> None:
